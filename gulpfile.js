...
async function screenshots(dirs = []) {
  ...
  for (i in sections) {
    section = sections[i];
    screenshot = section
      .replace(sectionsDir, screenshotDir)
      .replace(".html", ".png");
...
function sections(type = "section", dirs = []) {
  ...
  //sort sections
  dir.sort(
    (a, b) => sectionsSortOrder.indexOf(b) - sectionsSortOrder.indexOf(a)
  );
  ...
  for (i in dir) {
    ...
    let html;
    let sectionsHtml =
      '<html><head><base src="../"><link href="../css/style.css" rel="stylesheet"><link href="../css/screenshots.css" rel="stylesheet"></head><body>';
    let image = "";
    ...
    let files = fs
      ...
      .map((sectionFile) => {
        ...
        name = prettify(name);
        image =
          `screenshots/${group}/` + sectionFile.replace(".html", "-thumb.jpeg");
        html = fs.readFileSync(`${filePath}/${sectionFile}`, "utf8");
        sections.push(id);

        sectionsJs += template(templates[type]["section"], {
          id,
          name,
          image,
          html,
        });
        sectionsHtml += html.replace(
          "<section",
          '<section title="' + name + '"'
        );
      });
  ...
  return fs.writeFileSync(
    /*path.resolve(".") + */ /*`${sectionsDir}/${type}s.js`*/ path.resolve(
      "."
    ) + `/${type}s/${type}s.js`,
    sectionsJs
  );
...
gulp.task(
  "screenshots-blocks",
  gulp.series("connect", "take-screenshots-blocks")
);
